...
const vueJobDataSchema = recordedJdSchema
  ...
  .extend({
    ...
    encryptBossId: z.string().default(''), // HR 加密 id，取自 currentJob.encryptBossId（预留：直达会话链接用）
    securityId: z.string().default(''), // 会话安全 id，取自 currentJob.securityId，构造职位详情链接用
    encryptJobId: z.string().default(''), // 职位加密 id，取自 currentJob.encryptJobId，构造职位详情链接用
    lid: z.string().default(''), // 链接来源埋点标识，取自 currentJob.lid（预留），读不到为空串
  });
